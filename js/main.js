...
// Success Stories section functionality - CORRECTED LAYOUT & FADE
function initializeSuccessStories() {
  const storiesWrapper = document.querySelector('.success-stories-section .stories-wrapper');
  const storyCardsContainer = storiesWrapper ? storiesWrapper.querySelector('.story-cards-container') : null;
  const storyCards = storyCardsContainer ? storyCardsContainer.querySelectorAll('.success-card') : [];
  const prevButton = storiesWrapper ? storiesWrapper.querySelector('.prev-story') : null;
  const nextButton = storiesWrapper ? storiesWrapper.querySelector('.next-story') : null;
  const dotsContainer = storiesWrapper ? storiesWrapper.querySelector('.dots-container') : null;

  if (!storiesWrapper || !storyCardsContainer || storyCards.length === 0 || !prevButton || !nextButton || !dotsContainer) {
      console.log("Success stories elements not fully found.");
      return;
  }
  ...
  // --- Initial Setup ---
   function initialize() {
      storyCards.forEach((card, index) => {
          if (index === 0) {
              card.style.display = 'block'; // Show first card immediately
              card.style.opacity = '1';
              card.style.visibility = 'visible';
              card.classList.add('active'); // Mark as active
              // Animate results for the first card on load
              animateResults(card);
          } else {
  ...
  function updateStories(newIndex) {
      ...
      const oldCard = storyCards[oldIndex];
      const newCard = storyCards[newIndex];

      currentIndex = newIndex;

      // Start fading out the old card
      if (oldCard) {
      ...
      // Prepare the new card (make it display block but keep invisible)
      newCard.style.display = 'block'; // Make it take space for layout calculation if needed, but opacity is 0
      newCard.style.visibility = 'hidden'; // Keep hidden until transition starts
      newCard.style.opacity = '0'; // Ensure it starts transparent
      ...
      // Wait a tiny moment for display:block to register, then start fade-in
      requestAnimationFrame(() => {
          requestAnimationFrame(() => { // Double requestAnimationFrame for safety
               newCard.style.opacity = '1';
               newCard.style.visibility = 'visible';
               newCard.classList.add('active');
               // Animate results for the new card
               animateResults(newCard);
          });
      });


      // After the transition duration, hide the old card completely
  ...
  function animateResults(activeCard) {
      const resultValues = activeCard.querySelectorAll('.result-value');
      resultValues.forEach(el => {
          // Use a similar animation function as for specialty stats
          animateSingleStat(el);
      });
  }

  // --- Helper function to animate a single stat (reuse/adapt from specialties) ---
  function animateSingleStat(element) {
      const targetValueString = element.textContent || element.innerText;
      const match = targetValueString.match(/([-+]?\d+[\.,]?\d*)(.*)/);

      if (!match) {
          console.error("Could not parse result value:", targetValueString);
          element.textContent = targetValueString; // Reset to original if parsing fails
          return;
      }
      ...
      const isYears = suffix.toLowerCase().includes('an');

      if (isNaN(targetValue)) {
          console.error("Parsed target value is NaN for:", targetValueString);
           element.textContent = targetValueString; // Reset to original
          return;
      }
      ...
      // Store original value in a data attribute if not already done
      if (!element.dataset.originalValue) {
          element.dataset.originalValue = targetValueString;
      }
      // Reset to 0 before starting animation if needed (or another start value)
      ...
      const countUp = () => {
          ...
          } else if (targetValue % 1 !== 0 && !isPercentage) {
              formattedValue = currentValue.toFixed(1);
          } else {
              formattedValue = Math.round(currentValue);
          }
          ...
          // Add sign back if needed
           if (targetValue < 0 && parseFloat(formattedValue) > 0) {
               formattedValue = -formattedValue;
           }
            // Add '+' sign for positive years > 0
          if (isYears && targetValue > 0 && parseFloat(formattedValue) > 0) { // Check formattedValue > 0 to avoid "+0"
              formattedValue = '+' + formattedValue;
          }
          ...
          if (frame < totalFrames) {
              ...
              element.textContent = element.dataset.originalValue; // Ensure final value is exact
...
// Make sure the initialization function is called
document.addEventListener('DOMContentLoaded', function() {
  // ... other initializations ...
  initializeSuccessStories(); // Ensure this is called
  // ... other initializations ...
});
...
// --- Specialties Section - Offcanvas Panel Logic ---

function initializeSpecialtyOffcanvas() {
    const openButtons = document.querySelectorAll('.open-specialty-panel');
    const offcanvasElement = document.getElementById('specialtyOffcanvas');

    if (!openButtons.length || !offcanvasElement) {
        console.log("Offcanvas elements not found.");
        return;
    }

    // Create a Bootstrap Offcanvas instance
    const specialtyOffcanvas = new bootstrap.Offcanvas(offcanvasElement);

    // Titles mapping (adjust if your titles are different)
    const specialtyTitles = {
        'weight-loss': 'Perte de Poids Durable et Scientifique',
        'ibs': 'Syndrome du Côlon Irritable',
        'cardio': 'Hypertension & Cholestérol',
        'diabetes': 'Diabète de Type 2'
    ...
    openButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            const contentTemplateId = `content-${specialty}`;
            ...
            if (contentTemplate && offcanvasTitle && offcanvasBody) {
                // Set Title
                offcanvasTitle.textContent = specialtyTitles[specialty] || 'Détails du Service';

                // Set Body Content (Clone the content to avoid issues if opened multiple times)
                ...
                offcanvasBody.appendChild(clonedContent);
                 ...
                 // Optional: Re-initialize animations or specific JS for the loaded content if needed
                 // e.g., if blueprint stages need animation trigger *after* loading:
                 triggerInternalAnimations(offcanvasBody);


                // Show the offcanvas
                specialtyOffcanvas.show();
            } else {
                console.error(`Content template #${contentTemplateId} or Offcanvas elements not found.`);
                 offcanvasBody.innerHTML = '<p>Erreur: Contenu non trouvé.</p>';
                 specialtyOffcanvas.show(); // Show with error message
            }
        });
    });

    const mobileCloseBtn = offcanvasElement.querySelector('.offcanvas-close-btn-mobile');
    if (mobileCloseBtn) {
        mobileCloseBtn.addEventListener('click', function() {
            specialtyOffcanvas.hide(); // Use Bootstrap's hide method
        });
    }

     // Optional function to trigger animations within the newly loaded offcanvas content
     function triggerInternalAnimations(container) {
         const elementsToAnimate = container.querySelectorAll('.blueprint-stage, .modal-testimonial, .modal-cta-container');
          // Add a class to the container to scope the animation CSS
         container.parentElement.classList.add('offcanvas-visible'); // Add to offcanvas element
         elementsToAnimate.forEach((el) => {
             el.classList.add('animated');
         });
     }

     // Optional: Reset animations when offcanvas is hidden
      offcanvasElement.addEventListener('hidden.bs.offcanvas', function () {
         const elementsToAnimate = offcanvasElement.querySelectorAll('.blueprint-stage, .modal-testimonial, .modal-cta-container');
         offcanvasElement.classList.remove('offcanvas-visible');
          elementsToAnimate.forEach(el => {
...
// --- End Offcanvas Panel Logic ---

// Add the call inside the main DOMContentLoaded listener:
document.addEventListener('DOMContentLoaded', function() {
  // ... (existing initializations) ...

  initializeSpecialtyOffcanvas(); // <-- ADD THIS CALL
  // Remove initializeModalAnimations(); if it exists
